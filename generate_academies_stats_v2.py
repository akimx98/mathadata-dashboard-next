...
def normalize_academy_name(name: str) -> str:
    ...
    # Corrections pour version sans accent
    name_no_accent = remove_accents(name).lower()
    
    # Map des noms canoniques
    canonical_names = {
...
def main():
    ...
    print("🔄 Génération des statistiques académiques (V2 - depuis datasets élèves)...")
    print()
    
    # 1. Établissements (depuis dataset établissements pour les collèges uniquement)
    print("1️⃣  Comptage des collèges (depuis dataset établissements)...")
    records = call_api(
    ...
    # 2. Lycées GT et élèves GT (depuis dataset élèves GT)
    print("2️⃣  Comptage des lycées GT et élèves (depuis dataset élèves GT 2024-2025)...")
    records = call_api(
        'fr-en-lycee_gt-effectifs-niveau-sexe-lv',
        'count(distinct numero_lycee) as nb_lycees, sum(nombre_d_eleves) as total_eleves, academie',
    ...
    for rec in records:
        ...
        if academy not in stats:
            stats[academy] = {}
        stats[academy]['nb_lycees_gt'] = rec['record']['fields']['nb_lycees']
        stats[academy]['nb_eleves_lycees_gt'] = rec['record']['fields']['total_eleves']
    print(f"   ✓ {len(records)} académies")
    print()
    
    # 3. Lycées Pro et élèves Pro (depuis dataset élèves Pro)
    print("3️⃣  Comptage des lycées Pro et élèves (depuis dataset élèves Pro 2024-2025)...")
    records = call_api(
    ...
    for rec in records:
        ...
        if academy not in stats:
            stats[academy] = {}
        stats[academy]['nb_lycees_pro'] = rec['record']['fields']['nb_lycees']
